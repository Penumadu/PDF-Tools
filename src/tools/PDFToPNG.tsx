...
export const PDFToPNG: React.FC = () => {
  ...
  const handleFiles = async (files: File[]) => {
    ...
    try {
      ...
      for (let i = 1; i <= numPages; i++) {
        const blob = await pageToImageBlob(pdfProxy, i, 'image/png', 1, 2);
        converted.push({
          pageNum: i,
          blob,
  ...
  const downloadAllAsZip = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${file.name.replace(/\.[^/.]+$/, "")}-pngs.zip`;
    ...
    <ToolPage tool={tool}>
      {!isProcessing && !file && (
        <DropZone
          onFiles={handleFiles}
          multiple={false}
          label="Drop PDF here to convert to PNG"
          color={tool.color}
        ...
        <>
          <div className="result-section" style={{ background: tool.colorLight, borderColor: 'rgba(59, 130, 246, 0.2)' }}>
            ...
            <div className="result-subtitle" style={{ color: '#1e40af' }}>
              Converted {images.length} {images.length === 1 ? 'page' : 'pages'} to high-quality PNG format
            </div>
